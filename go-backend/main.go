...
func main() {
	...
	// Define routes with HTTPS enforcement, security headers, rate limiting on auth endpoints
	...
	mux.HandleFunc("/category/add", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addCategoryHandler))))
	mux.HandleFunc("/category/list", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listCategoryHandler))))
	mux.HandleFunc("/transaction/add", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addTransactionHandler))))
	mux.HandleFunc("/transaction/list", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listTransactionHandler))))
	mux.HandleFunc("/transaction/update", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, updateTransactionHandler))))
	mux.HandleFunc("/transaction/delete", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, deleteTransactionHandler))))
	mux.HandleFunc("/summary/totals", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryTotalsHandler))))
	mux.HandleFunc("/summary/monthly", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryMonthlyHandler))))
	mux.HandleFunc("/summary/current-month", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryCurrentMonthHandler))))
	mux.HandleFunc("/summary/category", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryCategoryHandler))))
	mux.HandleFunc("/summary/group", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryGroupHandler))))
	mux.HandleFunc("/summary/category/monthly", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, summaryCategoryMonthHandler))))
	mux.HandleFunc("/export", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, exportTransactionsHandler))))
	mux.HandleFunc("/recurring/add", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addRecurringHandler))))
	mux.HandleFunc("/recurring/list", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listRecurringHandler))))
	mux.HandleFunc("/recurring/edit", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, editRecurringHandler))))
	mux.HandleFunc("/recurring/delete", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, deleteRecurringHandler))))
	mux.HandleFunc("/transactions/search", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, searchAndFilterTransactionsHandler))))
	mux.HandleFunc("/budget/add", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addBudgetHandler))))
	mux.HandleFunc("/budget/list", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listBudgetHandler))))
	mux.HandleFunc("/budget/update", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, updateBudgetHandler))))
	mux.HandleFunc("/budget/delete", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, deleteBudgetHandler))))
	mux.HandleFunc("/budget/alerts", middleware.RequireHTTPS(middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, budgetAlertsHandler))))
