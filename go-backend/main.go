...
func main() {
	...
	// Define routes with security headers, rate limiting on auth endpoints
	mux.HandleFunc("/register", middleware.SecurityHeaders(rateLimitAuth(registerHandler)))
	mux.HandleFunc("/login", middleware.SecurityHeaders(rateLimitAuth(loginHandler)))
	// Protected routes (require JWT in Authorization header)
	mux.HandleFunc("/category/add", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addCategoryHandler)))
	mux.HandleFunc("/category/list", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listCategoryHandler)))
	mux.HandleFunc("/transaction/add", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addTransactionHandler)))
	mux.HandleFunc("/transaction/list", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listTransactionHandler)))
	mux.HandleFunc("/transaction/update", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, updateTransactionHandler)))
	...
	mux.HandleFunc("/export", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, exportTransactionsHandler)))
	mux.HandleFunc("/recurring/add", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addRecurringHandler)))
	mux.HandleFunc("/recurring/list", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listRecurringHandler)))
	mux.HandleFunc("/recurring/edit", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, editRecurringHandler)))
	mux.HandleFunc("/recurring/delete", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, deleteRecurringHandler)))
	mux.HandleFunc("/transactions/search", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, searchAndFilterTransactionsHandler)))
	mux.HandleFunc("/budget/add", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, addBudgetHandler)))
	mux.HandleFunc("/budget/list", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, listBudgetHandler)))
	mux.HandleFunc("/budget/update", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, updateBudgetHandler)))
	mux.HandleFunc("/budget/delete", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, deleteBudgetHandler)))
	mux.HandleFunc("/budget/alerts", middleware.SecurityHeaders(middleware.RequireAuth(jwtSecret, budgetAlertsHandler)))
